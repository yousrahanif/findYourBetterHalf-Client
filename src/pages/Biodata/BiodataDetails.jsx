...
const BiodataDetails = () => {
  ...
  const [isFavorite, setIsFavorite] = useState(false);
  const [favoriteIds, setFavoriteIds] = useState([]);
  const [userDetails, setUserDetails] = useState(null); // For storing user data from /users endpoint
    ...
    if (user?.email) {
      ...
      fetch(`http://localhost:5000/users`)
        ...
        .then((users) => {
          ...
          if (currentUser) {
            setUserDetails(currentUser); // Store user details
          }
        })
    ...
    <div className='mx-auto w-11/12'>
      ...
      <div className="flex items-center justify-center h-full">
        ...
        <div className="mb-2 mt-2.5 flex flex-col items-center"> 
          ...
          {userDetails?.member_type === 'premium' ? (
            <>
              <div className="flex items-center"> 
                <p className="mr-2">Contact Email: {contact_email}</p>
              </div>
